...
export const registerUser = (reqData) => async (dispatch) => {
  ...
  try {
    ...
    if(data.role==="ROLE_BUSINESS_OWNER"){
      reqData.navigate("/admin/business")
    }
    else{
      reqData.navigate("/")
    }
...
export const addToFavorites = ({businessId,jwt}) => {
  return async (dispatch) => {
    dispatch({ type: ADD_TO_FAVORITES_REQUEST });
    try {
      const { data } = await api.put(`api/businesses/${businessId}/add-favorites`,{},{
        headers: {

// ActionTypes.js

export const CREATE_BUSINESS_REQUEST = "CREATE_BUSINESS_REQUEST";
export const CREATE_BUSINESS_SUCCESS = "CREATE_BUSINESS_SUCCESS";
export const CREATE_BUSINESS_FAILURE = "CREATE_BUSINESS_FAILURE";

export const GET_ALL_BUSINESSS_REQUEST = "GET_ALL_BUSINESSS_REQUEST";
export const GET_ALL_BUSINESSS_SUCCESS = "GET_ALL_BUSINESSS_SUCCESS";
export const GET_ALL_BUSINESSS_FAILURE = "GET_ALL_BUSINESSS_FAILURE";

export const DELETE_BUSINESS_REQUEST = "DELETE_BUSINESS_REQUEST";
export const DELETE_BUSINESS_SUCCESS = "DELETE_BUSINESS_SUCCESS";
export const DELETE_BUSINESS_FAILURE = "DELETE_BUSINESS_FAILURE";

export const UPDATE_BUSINESS_REQUEST = "UPDATE_BUSINESS_REQUEST";
export const UPDATE_BUSINESS_SUCCESS = "UPDATE_BUSINESS_SUCCESS";
export const UPDATE_BUSINESS_FAILURE = "UPDATE_BUSINESS_FAILURE";

export const GET_BUSINESS_BY_ID_REQUEST = "GET_BUSINESS_BY_ID_REQUEST";
export const GET_BUSINESS_BY_ID_SUCCESS = "GET_BUSINESS_BY_ID_SUCCESS";
export const GET_BUSINESS_BY_ID_FAILURE = "GET_BUSINESS_BY_ID_FAILURE";

export const GET_BUSINESS_BY_USER_ID_REQUEST =
  "GET_BUSINESS_BY_USER_ID_REQUEST";
export const GET_BUSINESS_BY_USER_ID_SUCCESS =
  "GET_BUSINESS_BY_USER_ID_SUCCESS";
export const GET_BUSINESS_BY_USER_ID_FAILURE =
  "GET_BUSINESS_BY_USER_ID_FAILURE";

export const UPDATE_BUSINESS_STATUS_REQUEST =
  "UPDATE_BUSINESS_STATUS_REQUEST";
export const UPDATE_BUSINESS_STATUS_SUCCESS =
  "UPDATE_BUSINESS_STATUS_SUCCESS";
export const UPDATE_BUSINESS_STATUS_FAILURE =
  "UPDATE_BUSINESS_STATUS_FAILURE";

  export const CREATE_EVENTS_REQUEST = "CREATE_EVENTS_REQUEST";
  export const CREATE_EVENTS_SUCCESS = "CREATE_EVENTS_SUCCESS";
  export const CREATE_EVENTS_FAILURE = "CREATE_EVENTS_FAILURE";

  export const GET_ALL_EVENTS_REQUEST = "GET_ALL_EVENTS_REQUEST";
  export const GET_ALL_EVENTS_SUCCESS = "GET_ALL_EVENTS_SUCCESS";
  export const GET_ALL_EVENTS_FAILURE = "GET_ALL_EVENTS_FAILURE";

  export const DELETE_EVENTS_REQUEST = "DELETE_EVENTS_REQUEST";
  export const DELETE_EVENTS_SUCCESS = "DELETE_EVENTS_SUCCESS";
  export const DELETE_EVENTS_FAILURE = "DELETE_EVENTS_FAILURE";

  export const GET_RESTAIRANTS_EVENTS_REQUEST = "GET_RESTAIRANTS_EVENTS_REQUEST";
  export const GET_RESTAIRANTS_EVENTS_SUCCESS = "GET_RESTAIRANTS_EVENTS_SUCCESS";
  export const GET_RESTAIRANTS_EVENTS_FAILURE = "GET_RESTAIRANTS_EVENTS_FAILURE";

  export const CREATE_CATEGORY_REQUEST = "CREATE_CATEGORY_REQUEST";
  export const CREATE_CATEGORY_SUCCESS = "CREATE_CATEGORY_SUCCESS";
  export const CREATE_CATEGORY_FAILURE = "CREATE_CATEGORY_FAILURE";

  export const GET_BUSINESSS_CATEGORY_REQUEST = "GET_BUSINESSS_CATEGORY_REQUEST";
  export const GET_BUSINESSS_CATEGORY_SUCCESS = "GET_BUSINESSS_CATEGORY_SUCCESS";
  export const GET_BUSINESSS_CATEGORY_FAILURE = "GET_BUSINESSS_CATEGORY_FAILURE";
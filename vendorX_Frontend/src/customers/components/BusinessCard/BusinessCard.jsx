...
const BusinessCard = ({ data, index }) => {
  ...
  const handleAddToFavorites = () => {
    dispatch(addToFavorites({businessId:data.id,jwt:auth.jwt||jwt}));
  };

  const navigateToBusiness = () => {
    if(data.open)
    navigate(`/business/${data.address.city}/${data.name}/${data.id}`);
  };

  return (
    <Card className="m-5 w-[18rem] productCard ">
      <div onClick={navigateToBusiness} className={`${data.open?"cursor-pointer":"cursor-not-allowed"}  relative`}>
        <img

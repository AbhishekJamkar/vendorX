...
const MenuItemTable = ({ isDashboard, name }) => {
  ...
  const { menu, ingredients, business,auth } = useSelector((store) => store);
  const { id } = useParams();
  ...
  useEffect(() => {
      ...
      if(business.usersBusiness){
       dispatch( getMenuItemsByBusinessId({
  ...
  }, [ingredients.update,business.usersBusiness]);
  ...
  const handleProductAvialability = (productId) => {
    dispatch(updateMenuItemsAvailability({productId,jwt:auth.jwt || jwt}));
  };
    ...
    <Box width={"100%"}>
      <Card className="mt-1">
        <CardHeader
          ...
          action={
            <IconButton onClick={() => navigate("/admin/business/add-menu")}>
              <Create />
            </IconButton>
          }
        />
